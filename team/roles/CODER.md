...
- **All produced code follows `docs/coding-standards.md`.** That document is the authoritative list of coding directives (modularity, thin surfaces, shared `lib/`, no hardcoded values, `--help` correctness, comments describing behavior not process history, and the rest); rules that apply to any code you write are stated there, not restated here.
...
- **Helper-migration refactors: verify every migrated call site in the gated integration suite before DONE.** When a task centralises invocations behind a shared helper — a new `pp_run_ops`-style wrapper, a common shell function, an env-bootstrap sourcer, any factored-out call shape that replaces N direct invocations with N helper calls — every migrated call site must be exercised end-to-end by `team/scripts/tests/run-integration-tests.sh` (in addition to `run-unit-tests.sh`) in your own local run before you mark DONE. Unit tests can pass while an integration-only path (a wrong-root helper path, a missing `source`, a KANBAN_ROOT-vs-dev-tree-root confusion) silently breaks the migrated site at deployment time. If any migrated call site is not covered by the existing integration suite, extend the suite in the same task so coverage exists before merge. This closes the failure mode that produced the mid-RC operator hot-patch on `halt-global.sh` / `unhalt-global.sh` in v1.23.12: seven integration failures introduced by a centralisation refactor that would have been caught by a local integration run before DONE.
...
The boundary that comments, docstrings, `--help` text, log lines, and output strings describe behavior rather than process history is owned by `docs/coding-standards.md` (directive 8); read it there for the rule and its exceptions.
