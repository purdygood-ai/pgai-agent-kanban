...
# Codex provider peer to wake-claude.sh.
#
# Usage:
#   wake-codex.sh --agent=<agent> [--sleep=N] [--help]
#   wake-codex.sh -a <agent> [-s N] [-h]
#   wake-codex.sh <agent>    (deprecated positional form; emits a warning)
#
# Options:
...
#
# Exits immediately (< 1 second) when active-provider != 'codex'.
# This fast exit is required for cron scheduling so inactive providers add no overhead.
#
...
_print_usage() {
  cat >&2 <<USAGE
Usage: $(basename "$0") --agent=AGENT [--sleep=N] [--help]
...
# --- Source active_provider.sh for read_active_provider ---
# Must be sourced before the provider check below and before wake_common.sh.
# shellcheck source=scripts/lib/active_provider.sh
source "${SCRIPT_DIR}/../lib/active_provider.sh"

# --- Read active provider and exit immediately if not codex ---
# Done before provider_preflight and wake_common.sh to guarantee < 1 second
# exit when this wake script is not the active provider.
# Inactive-provider fast exit: not the active provider, exit immediately.
...
if [[ "$ACTIVE_PROVIDER" != "codex" ]]; then
  ...
  echo "[$(date -Iseconds)] wake-codex(${AGENT}): active-provider is '${ACTIVE_PROVIDER}', not 'codex' — exiting immediately" >&2
  exit 0
fi

# --- Source provider lib (Codex-specific: codex_parse_jsonl_result, provider_invoke_agent) ---
# Must be sourced AFTER the active-provider check (to avoid loading Codex-specific
# state when this script exits early) and BEFORE wake_common.sh (so
# provider_invoke_agent is defined when wake_common.sh's functions reference it).
# wake_codex_provider.sh sources token_capture.sh internally; no need to source
# it separately here.
# shellcheck source=scripts/lib/wake_codex_provider.sh
source "${SCRIPT_DIR}/../lib/wake_codex_provider.sh"

# --- Preflight: verify Codex CLI is available ---
provider_preflight
...
# worktree.sh depends on temp.sh; wake_common.sh sources temp.sh so the
# include guard inside worktree.sh makes this idempotent.
...
if [[ "$AGENT" == "overwatch" ]]; then
  if [[ -f "${TEAM_ROOT}/HALT_OVERWATCH" ]]; then
    echo "[$(date -Iseconds)] wake-codex(${AGENT}): HALT_OVERWATCH flag detected — exiting cleanly" >&2
    exit 0
  fi
else
  if [[ -f "${TEAM_ROOT}/HALT" ]]; then
    echo "[$(date -Iseconds)] wake-codex(${AGENT}): HALT flag detected — exiting cleanly" >&2
    exit 0
  fi
fi
...
# Sanity check: if ai_auth_mode=oauth but OPENAI_API_KEY is still set after
# sourcing secrets, warn the operator — the Codex CLI may prefer OPENAI_API_KEY
# over OAuth credentials when the env var is set.
if [[ "${AI_AUTH_MODE:-oauth}" == "oauth" && -n "${OPENAI_API_KEY:-}" ]]; then
    log "WARNING: AI_AUTH_MODE=oauth but OPENAI_API_KEY is set and non-empty. If you intend to use OAuth, remove or comment out OPENAI_API_KEY from your secrets file to avoid credential conflicts."
fi

# ---------------------------------------------------------------------------
# Codex-provider substrate function overrides.
#
# These definitions override the provider-neutral defaults in wake_common.sh
# with Codex-specific versions. Because bash last-definition-wins, these
# take effect at runtime while wake_common.sh retains neutral defaults.
# ---------------------------------------------------------------------------
...
# --- Process exactly one task (Codex provider override) ---
# Calls provider_invoke_agent (defined by wake_codex_provider.sh) for CLI invocation.
LAST_TASK_FINAL_STATE=""
process_one_task() {
  local task_id task_dir task_readme task_status task_log_dir task_artifact_dir
  local current_state final_state codex_exit prompt role SUBAGENT
  ...
  if [[ -z "${task_id:-}" ]]; then
    log "no pending Codex task found in ${AGENT} queue"
    return 1
  fi
  ...
  # --- Per-agent debug log path ---
  # Route to projects/<project>/logs/debug/<agent>.log so this
  # provider override matches the wake_common.sh substrate (last-def-wins).
  # Falls back to the global path only if _CURRENT_PROJECT is somehow unset.
  ...
  # Resting branch the parking step checked out (or empty when
  # parking was skipped — sandbox guard or no dev tree).  Restored after the
  # worktree is torn down so the canonical dev tree is never left parked or
  # detached after a task.
  ...
  if [[ "$SUBAGENT" == "coder" || "$SUBAGENT" == "writer" ]]; then
    ...
    if [[ "$_task_workflow_type" != "document" ]]; then
      ...
      # Use the project's resting branch (prefix+develop, then
      # prefix+main) via pp_prefix_branch, falling back to --detach only when
      # neither prefixed branch exists locally.  The resting branch is recorded
      # in _park_resting_branch and restored after the worktree is torn down.
      #
      # Sandbox guard: skip parking when the kanban root (TEAM_ROOT)
      # is under a temp directory.  Integration tests build a synthetic kanban
      # root under /tmp (pytest tmp_path) but point dev_tree_path at the real
      # dev tree.  Without this guard the parking step mutates the real dev tree,
      # breaking subsequent tests.  Real operator installs sit at
      # $HOME/pgai_agent_kanban and are never under /tmp, so production parking
      # is unaffected.
      # Resolve per-project dev tree first; fall back to global for
      # single-project / legacy installs where PP_dev_tree_path is unset.
      ...
      # Capture ONLY stdout (the resolved worktree path). Do NOT use
      # 2>&1 here — that merges stderr into the captured value, allowing git
      ...
      # The git worktree add inside the helper now uses >/dev/null 2>/dev/null
      # so git's own progress messages are fully suppressed at the source.
      local _ct_exit=0
      set +e
...
Use this exact name when running: git checkout -b"
    ...
    #
    # TESTER parks the canonical dev tree onto the project's
    # resting branch (${prefix}develop → ${prefix}main → --detach) and restores
    # it after the task, so the dev tree is never left parked or detached.
    local _wt_exit=0
    set +e
    _task_workflow_type="$(get_workflow_type_from_readme "$task_readme")"
    _wt_exit=$?
    ...
    if [[ "$_task_workflow_type" != "document" ]]; then
      ...
      # Park the canonical dev tree onto the project's resting branch
      # before creating the detached worktree, and record the branch for restore.
      ...
      # Capture ONLY stdout (the resolved worktree path). Do NOT use
      # 2>&1 here — stderr from create_detached_worktree flows to the wake-script
      # stderr/cron log directly. git's own progress messages are suppressed at
      # the source inside worktree.sh (>/dev/null 2>/dev/null).
      local _ct_exit=0
      set +e
...
Your working directory is an isolated git worktree at ${_task_worktree_path} checked out in detached HEAD mode at the RC branch ${_task_source_branch}. No feature branch was created. Do NOT run 'git checkout -b'. PGAI_DEV_TREE_PATH points at this worktree for the duration of the task."
  ...
  prompt=$(cat <<EOF
Process the kanban task at ${task_dir}.
...
Do not over-log. One note per significant action, not one per line of code. The operator wants to know roughly where you are in the task, not a transcript of your reasoning."
  ...
  # Gate on the same condition as the Stage-2 copy block (~line 619) so that
  ...
  # Mirrors wake_common.sh lines 1705-1716 and claude.sh injection block.
...
This trace is for human prompt-engineering analysis, not for orchestration. It does not affect task state, queue progression, or your task outcome. Write it as honest introspection rather than performance."
  ...
  local _pollution_snapshot_ok=false
  {
    if [[ -d "${_sweep_canonical_tree:-}" ]]; then
      ...
      # at info level.  Reserve WARNING for release-workflow projects whose configured
  ...
  # provider_invoke_agent is defined by the sourced provider lib (wake_codex_provider.sh).
  ...
  if [[ "$SUBAGENT" == "tester" && -n "${_task_worktree_path:-}" ]]; then
    ...
    # The worktree teardown below removes the detached worktree; this restores the
    # canonical HEAD so the operator finds the dev tree on its resting branch, not
    # parked or detached.
    if [[ -n "${_park_resting_branch:-}" && -n "${_canonical_dev_tree_path:-}" && -d "${_canonical_dev_tree_path}/.git" ]]; then
      set +e
      git -C "$_canonical_dev_tree_path" checkout "$_park_resting_branch" 2>/dev/null
...
PY_POL
  ...
  # round-trip through the env-variable path.
  if [[ -n "${_task_worktree_path:-}" ]]; then
  ...
  # post-teardown).  For CODER/WRITER tasks the env-var restore above is not needed
  # (TESTER is the only path that overrides PGAI_DEV_TREE_PATH), but the git HEAD
  # MUST be restored here so the dev tree is never left parked after the worktree
  # is torn down.  The TESTER restore already ran above; this block is for
  # CODER/WRITER tasks only (guard: SUBAGENT != tester).
...
# ---------------------------------------------------------------------------
# run_project_chain <project_name> (Codex provider override)
...
run_project_chain() {
  ...
  _preflight_model="${!_agent_model_var:-}"   # env-var model; empty = config default
